...
Current directory: Run 'pwd' to see your location"""
    ...
    def agent_import_error(filename: str, error: Exception) -> str:
        ...
        if "ModuleNotFoundError" in str(type(error)):
            suggestions.insert(0, "Install missing dependencies (check requirements.txt)")
...
To fix:
  ...
  3. Wait a moment and try again"""
    ...
    def connection_error(error: Exception, attempt: int, max_retries: int) -> str:
        ...
        return f"""{Colors.error(f'⚠️  Connection error (attempt {attempt}/{max_retries})')}
...
To fix:
  {suggestion_text}"""
    ...
    @staticmethod
    def config_file_error(filepath: str, error: Exception, operation: str = "read") -> str:
